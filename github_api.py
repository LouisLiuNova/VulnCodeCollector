...
def fetch_patch_source_code(cve_number: str, commit_url: str):
    ...
    try:
        ...
        res = requests.get(
            f"{BASE_ENDPOINT}/repos/{info['owner']}/{info['repo']}/commits/{info['commit_sha']}", headers=headers)
    ...
    for file in info["files"]:
        ...
        if file['status']=="deleted":
            ...
            logger.info(f"File {file_name} is added so skip fetching the previous version.")
            ...
            fetch_file_content(cve_number,info["owner"],info["repo"],file["filename"],info["parent_commit_sha"]) # Fetch the previous(vulnerable) version of the file
            ...
            pathlib.Path(f"data/{cve_number}/patched").mkdir(parents=True, exist_ok=True)
            with open(f"data/{cve_number}/patched/{filename}", "w") as f:
                f.write(diff_content)
            logger.info(f"Successfully saved diff file {filename}.")

        else:
            logger.warning(f"Skip file {file_name} since it's in unrecognized status {file['status']}.")
            continue
...
def fetch_file_content(cve_number:str,owner:str, repo:str, file_path:str, commit_sha:str):
    """
    ...
    try:
        ...
        res = requests.get(
            f"{BASE_ENDPOINT}/repos/{owner}/{repo}/contents/{file_path}?ref={commit_sha}", headers=headers)
        res.raise_for_status()
        file_info = res.json()
    except requests.exceptions.HTTPError as err:
    ...
    # Check if the directory exists
    pathlib.Path(f"data/{cve_number}/vulnerable").mkdir(parents=True, exist_ok=True)
    with open(f"data/{cve_number}/vulnerable/{filename}", "w") as f:
        f.write(file_content)
...
def resolve_commit_url(commit_url: str):
    ...
    paths = parsed_url.path.split("/")[1:]  # NOTE: should be like ['LibRaw', 'LibRaw', 'commit', '4606c28f494a750892c5c1ac7903e62dd1c6fdb5']. [1:] is to strip the first empty segment.
    res["owner"] = paths[0]
    res["repo"] = paths[1]
